...
if($view=='list'){
    ...
    if($rs=$dop->getRecs()){ 
        foreach($rs as $r){ 
          ...
          $rc = exaHouse::getRCnts($r, '1', 'rent'); 
          echo "<tr>\n";
          echo $cv->Select($did);
          $lp = exaHouse::getLprow($r['lpid'], 'house');
          echo $cv->Title($r,1,'title','',32,"<br><span class='c666'>$lp[lparea] / $lp[lplink]</span>");
          echo $cv->Types($r['catid']);
          echo $cv->Show($r['show']);
          $zutype = $cv->TKeys($r,0,'zutype',12,"<i class='cCCC'>类型</i>");
          $zxtype = $cv->TKeys($r,0,'zxtype',12,"<i class='cCCC'>装修</i>");
          $wytype = $cv->TKeys($r,0,'wytype',12,"<i class='cCCC'>物业</i>");
          echo "<td class='tl'>
          <a href='?dops-a&mod=album&lpid={$r['did']}&part=rent'>相册($rc[cntpic])</a> #
          {$r['hxs']} <br> $zutype / $zxtype / $wytype
          </td>\n";
    ...
    if(!empty($bsend)){
        ...
        $dop->fmv['lpname'] = $dop->fme['lpid_refname']; // 使用一个未用的字段保存楼盘名称 
