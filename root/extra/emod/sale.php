...
if($view=='list'){
    ...
    if($rs=$dop->getRecs()){ 
        foreach($rs as $r){ 
          ...
          echo $cv->Show($r['show']);
          $sale = $cv->TKeys($r,0,'sales',12,"<i class='cCCC'>状态</i>");
          $zxtype = $cv->TKeys($r,0,'zxtype',12,"<i class='cCCC'>装修</i>");
          $wytype = $cv->TKeys($r,0,'wytype',12,"<i class='cCCC'>物业</i>");
          echo "<td class='tl'>
          <a href='?dops-a&mod=album&lpid={$r['did']}&part=sale'>相册($rc[cntpic])</a> #
          {$r['hxs']} <br> $sale / $zxtype / $wytype
          </td>\n";
